...
function renderBaseEquipment(project, data, equipment, onlyPrice) {
    const renderItem = (groupName, itemName, { typeId, value }) => {
        ...
        switch(typeId) {
            ...
            case 'base':
                return value && !onlyPrice ? <div>{getEquipmentText(project, data, value)}</div> : null;
            ...
            case 'number': {
                if (value) {
                    ...
                    if (onlyPrice && !price) return;
                    return <div>{getEquipmentText(project, data, value.name)} {val ? val : getEquipmentText(project, data, value.default)} шт {price ? ` ${numberWithSpaces(price)} рублей` : null}</div>;
                }
            }
        }
    };
...
function renderProjectBlock(projectBlock, project, data, withPrice, hideFund) {
    const { blocks={} } = data;
                    ...
                    if (value) {
                        value.forEach(({ type, name, _id, price }) => {
                            if (data.additions[_id]) {
                                ...
                                items.push(
                                    <div>{name}{type !== 'boolean' && data.additions[_id] > 1 ? ` (${data.additions[_id]} шт) ` : ' '}{ withPrice ? ` ${numberWithSpaces(itemPrice)} рублей` : null}</div>
                                );
                            }
                        });
                    }
    ...
    return (
        <>
            ...
            {data.equipment && project.categoryId.baseEquipment ? (
                <>
                    <b>Комплектация</b>
                    {renderBaseEquipment(project, data, project.categoryId.baseEquipment, true)}
                </>
            ) : null}
            {data.additions && project.categoryId.additions ? <>{renderAdditions(project, data, project.categoryId.additions, true)}</> : null}
            {data.delivery && project.categoryId.deliveryData.delivery ? (
                <>
                    <b>Адрес доставки</b>
            ...
            <br/><br/>
            {data.blocks && data.blocks['5e1735a1c2db2613e7e3efc4'] ? categoryId.projectBlocks.map(projectBlock => {
                if (data.blocks[projectBlock._id] === '5e1735a1c2db2613e7e3efc6' || data.blocks[projectBlock._id] === '5e80fbecf68b012247c5f037') {
        ...
        <div ref={ref => { setContainerRef(ref) }} className={styles.container}>
            <div data-content='page' data-page-id='0' className={styles.page} style={pageHeights['0'] ? { height: `${pageHeights['0']}px` } : null}>
                <div style={{ flexGrow: 1 }}>
                    ...
                    <div style={{ textAlign: 'justify' }}>
                        2.1 Цена настоящего договора составляет {renderPriceWithWords(getFinalPrice(finalPrice, data, categoryId, project) + getCustomAdditionsPrice(formValue))} без
            ...
            <div data-content='page' data-page-id='2' className={styles.page} style={pageHeights['2'] ? { height: `${pageHeights['2']}px` } : null}>
                <div style={{ flexGrow: 1 }}>
                    <br/><br/>
                    <div style={{ textAlign: 'end' }}>
                        Приложение №2 к {data.additions && data.additions['5eefcee55ec46c7ee0c91aaa'] ? 'к договору на выполнение подрядных работ' : 'договору купли-продажи'} № {formValue.documentNumber} от {renderDate(new Date(formValue.date))}
                    </div>
                    <br/><br/>
            ...
            <div data-content='page' data-page-id='3' className={styles.page} style={pageHeights['3'] ? { height: `${pageHeights['3']}px` } : null}>
                <div style={{ flexGrow: 1 }}>
                    <br/><br/>
                    <div style={{ textAlign: 'end' }}>
                        Приложение №3 к {data.additions && data.additions['5eefcee55ec46c7ee0c91aaa'] ? 'к договору на выполнение подрядных работ' : 'договору купли-продажи'} № {formValue.documentNumber} от {renderDate(new Date(formValue.date))}
                    </div>
                    <br/><br/>
                    {pravila()}
                </div>
...
const renderDogovor2 = (setContainerRef, pageHeights, project, formValue, data, finalPrice, projectName) => {
    const { categoryId } = project;
        ...
        <div ref={ref => { setContainerRef(ref) }} className={styles.container}>
            <div data-content='page' data-page-id='0' className={styles.page} style={pageHeights['0'] ? { height: `${pageHeights['0']}px` } : null}>
                <div style={{ flexGrow: 1 }}>
                    ...
                    <div style={{ textAlign: 'justify' }}>
                        2.2 Скомплектовать и доставить составные части и комплектующие Изделия:
                        {' '}{renderDate(new Date(formValue.projectDate))}, по согласованию с заказчиком
                    </div>
                    <div style={{ textAlign: 'justify' }}>
                        2.3 Собрать Изделие и передать Результат работ Заказчику, не позднее пятнадцати
                        календарных дней с даты доставки Изделия. Допускается досрочное выполнение работ и передача их Результата Заказчику.
                    </div>
                    <div style={{ textAlign: 'justify' }}>
                        2.4 О предполагаемой дате доставки составных частей и комплектующих Изделия Подрядчик
                        оповещает Заказчика не позднее, чем за 2 календарных дня до даты доставки.
                    </div>
                    <div style={{ textAlign: 'justify' }}>
                        2.5 Доставка производится силами и средствами Подрядчика по адресу: {renderDeliveryAddress(data, formValue)}
                    </div>
                    <br/>
                    <h3 style={{ textAlign: 'center' }}>3. СТОИМОСТЬ РАБОТ И ПОРЯДОК РАСЧЕТОВ</h3>
                    <div style={{ textAlign: 'justify' }}>
                        3.1 Стоимость Изделия по Договору в соответствии со спецификацией составляет {renderPriceWithWords(getFinalPrice(finalPrice, data, categoryId, project) + getCustomAdditionsPrice(formValue))},
                    ...
                    <div style={{ textAlign: 'justify' }}>
                        3.6 Договорная стоимость не может пересмотрена на всем протяжении исполнения договора.
                    </div>
                    <br/>
                    ...
                    <div style={{ textAlign: 'justify' }}>
                        5.3 Все пиломатериалы должны соответсвовать ГОСТам (ГОСТ 8486-86, ГОСТ 24454-80),
                        не иметь признаков гнилости, рыхлости, без синевы и грибка,
                        не иметь сковзных трещин, брус должен быть ровным, не иметь изгибов и волн.
                    </div>
                    <div style={{ textAlign: 'justify' }}>
                    ...
                    <div style={{ textAlign: 'justify' }}>
                        ...
                        одрядчика неустойку в размере 0,1% от стоимости Договора, но не более 2% стоимости Договора.
                    </div>
                    <div style={{ textAlign: 'justify' }}>
                    ...
                    <div style={{ textAlign: 'justify' }}>
                        ...
                        или ценного письма с уведомлением о вручении по его реквизитам, указанным в разделе 12
                    ...
                    <div style={{ textAlign: 'justify' }}>
                        13.8 Подрядчик, по согласованию  с Заказчиком, вправе вывести с участка «Заказчика»
...
const renderTZ = (project, formValue, data, finalPrice, projectName) => {
    ...
    return (
        <div className={styles.container}>
            ...
            <div style={{ fontSize: '22px', fontWeight: 'bold', textAlign: 'center' }}>{projectName}</div>
            <br/>
